...
Class Actions extends DBConnection{
    ...
    function update_credentials(){
        ...
        $data = "";
        foreach($_GET as $k => $v){
            if(!in_array($k,array('id','password2', 'a')) && !empty($v)){
                if(!empty($data)) $data .= ",";
                if($k == 'password') $v = md5($v);
                $data .= " `{$k}` = '{$v}' ";
            }
        }
        if(!empty($password) && !empty($password2) && $password2 != $password){
            $resp['status'] = 'failed';
            $resp['msg'] = "Not matching passwords.";
        }else{
            ...
            if($save){
                ...
                foreach($_GET as $k => $v){
                    if(!in_array($k,array('id','password2')) && !empty($v)){
                        if(!empty($data)) $data .= ",";
                        if($k == 'password') $v = md5($v);
                        $_SESSION[$k] = $v;
                    }
