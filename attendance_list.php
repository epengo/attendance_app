                    ...
                    <div class="col-lg-4 col-md-4 col-sm-6">
                        <div class="form-group">
                            ...
                            <select name="course_id" id="course_id" class="form-select form-select-sm rounded-0">
                                ...
                                <?php
                                $dept_qry = $conn->query("SELECT * FROM course_list where `status` = 1 order by `name` asc");
    ...
    <div class="card-body">
        <table class="table table-bordered table-hover" id="att-list">
            ...
            <tbody>
                <?php 
                $where = "";
                if(isset($course_id) && $course_id != "All")
                $where .= " s.course_id = '{$course_id}' and ";
                if(isset($yl_id))
                $where .= " s.yl_id = '{$yl_id}' and ";
                 $att_qry = $conn->query("SELECT a.date_created, a.type, (s.lastname || ', ' || s.firstname || ' ' || s.middlename),s.student_code FROM `attendance_list` a inner join student_list s on a.student_id = s.student_id where {$where} s.sy_id= '{$_SESSION['sy_id']}' order by a.date_created asc ");
                 $i = 1;
                ...
                <tr>
                    ...
                    <td class="align-middle py-0 px-1">
                        <p class="m-0">
                            <small><b>Student Code:</b> <?php echo $row[3] ?></small><br>
                            <small><b>Name:</b> <?php echo $row[2] ?></small>
                        </p>
                    </td>
                    <td class="align-middle py-0 px-1 text-center">
                        <span class="badge bg-<?php echo $bg ?>"><?php echo $row[1] == 1 ? "Time In" : "Time Out" ?></span>
                    </td>
                    <td class="align-middle py-0 px-1 text-end"><?php echo date("M d, Y h:i A",strtotime($row[0]))  ?></td>
                </tr>
                <?php endwhile; ?>
